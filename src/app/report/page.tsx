...
export async function getServerSections() {
  const res = await fetch(`${process.env.BACKEND_URL}/section`, {
    credentials: "include",
    next: { revalidate: 900 },
  });
  if (!res.ok) {
...
export async function getServerEntities() {
  const res = await fetch(`${process.env.BACKEND_URL}/section/name`, {
    credentials: "include",
    next: { revalidate: 900 },
  });
  if (!res.ok) {
...
export async function getServerBuildings() {
  const res = await fetch(`${process.env.BACKEND_URL}/entities/name`, {
    credentials: "include",
    next: { revalidate: 900 },
  });
  if (!res.ok) {
